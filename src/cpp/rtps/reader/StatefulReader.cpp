    ...
    if (is_alive_)
    {
                ...
                ++it)
        {
            if ((*it)->guid() == writer_guid)
            {
                ...
                remove_persistence_guid(wproxy->guid(), wproxy->get_attributes()->persistence_guid());
                break;
            }
        }
    ...
    if (acceptMsgFrom(change->writerGUID, &pWP))
    {
        if (liveliness_lease_duration_ < c_TimeInfinite)
        {
            if (liveliness_kind_ == MANUAL_BY_TOPIC_LIVELINESS_QOS ||
                    pWP->get_attributes()->m_qos.m_liveliness.kind == MANUAL_BY_TOPIC_LIVELINESS_QOS)
            {
                auto wlp = this->mp_RTPSParticipant->wlp();
                if (wlp != nullptr)
                {
    ...
    if (acceptMsgFrom(writerGUID, &writer) && writer)
    {
                    ...
                    hbCount, firstSN, lastSN, finalFlag, livelinessFlag, disable_positive_acks_, assert_liveliness))
        {
            ...
            if (assert_liveliness)
            {
                ...
                {
                    if (liveliness_kind_ == MANUAL_BY_TOPIC_LIVELINESS_QOS ||
                            writer->get_attributes()->m_qos.m_liveliness.kind == MANUAL_BY_TOPIC_LIVELINESS_QOS)
                    {
                        auto wlp = this->mp_RTPSParticipant->wlp();
                        if ( wlp != nullptr)
                        {

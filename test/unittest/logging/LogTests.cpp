...
int LogTests::send_logs_and_wait()
{
    logFatal(LogMacrosCheck, "Fatal Message");
    logCritical(LogMacrosCheck, "Critical Message");
    logError(LogMacrosCheck, "Error Message");
    logUser(LogMacrosCheck, "User Message");
    logWarning(LogMacrosCheck, "Warning Message");
    logInfo(LogMacrosCheck, "Info Message");
    logDebug(LogMacrosCheck, "Debug Message");
    auto consumedEntries = LogTests::HELPER_WaitForEntries(7);

    return consumedEntries.size();
}

TEST_F(LogTests, new_macros_functionality)
{
    std::cout << "------ Verbosity Fatal ------" << std::endl;
    Log::SetVerbosity(Log::Kind::Fatal);
    ASSERT_EQ(1u, send_logs_and_wait());

    std::cout << "------ Verbosity Critical ------" << std::endl;
    Log::SetVerbosity(Log::Kind::Critical);
    ASSERT_EQ(3u, send_logs_and_wait());

    std::cout << "------ Verbosity Error ------" << std::endl;
    Log::SetVerbosity(Log::Kind::Error);
    ASSERT_EQ(6u, send_logs_and_wait());

    std::cout << "------ Verbosity User ------" << std::endl;
    Log::SetVerbosity(Log::Kind::User);
    ASSERT_EQ(10u, send_logs_and_wait());

    std::cout << "------ Verbosity Warning ------" << std::endl;
    Log::SetVerbosity(Log::Kind::Warning);
    ASSERT_EQ(15u, send_logs_and_wait());

    std::cout << "------ Verbosity Info ------" << std::endl;
    Log::SetVerbosity(Log::Kind::Info);
    ASSERT_EQ(21u, send_logs_and_wait());

    std::cout << "------ Verbosity Debug ------" << std::endl;
    Log::SetVerbosity(Log::Kind::Debug);

#if (defined(__INTERNAL_DEBUG) || defined(_INTERNAL_DEBUG))
    std::cout << "Internal Debug Activated" << std::endl
#else
    std::cout << "No Internal Debug" << std::endl;
#endif // if (defined(__INTERNAL_DEBUG) || defined(_INTERNAL_DEBUG))

#if (defined(__DEBUG) || defined(_DEBUG))
    std::cout << "Debug Activated" << std::endl
#else
    std::cout << "No Debug" << std::endl;
#endif // if (defined(__DEBUG) || defined(_DEBUG))

    ASSERT_EQ(28u, send_logs_and_wait());
}
